...
MyTitle = ig.Game.extend({
	...
	init: function() {
        ...
        this.animSheet = new ig.AnimationSheet( 'media/title.png', 960, 540);
		this.anim = new ig.Animation( this.animSheet, 0.2, [0,1] );
	...
	update: function() {
		...
		if( ig.input.pressed('jump') || ig.input.pressed('start')) {
			ig.system.setGame( MyInstructions );
			return;
		}
	...
	draw: function() {
		this.parent();
        this.anim.draw(0,0);
        this.font.draw( 'PRESS START / X', 450, 250 )
	}
});


MyInstructions = ig.Game.extend({
	clearColor: "#d0f4f7",
	gravity: 800,

	// The title image
	title: new ig.Image( 'media/instructions.png' ),
	...
	init: function() {
		...
		ig.input.bind( ig.GAMEPAD.FACE_3, 'shoot' );	
        ig.input.bind( ig.GAMEPAD.START, 'start');
        this.animSheet = new ig.AnimationSheet( 'media/instructions.png', 960, 540);
		this.anim = new ig.Animation( this.animSheet, 0.2, [0] );
		// Align touch buttons to the screen size, if we have any
		if( window.myTouchButtons ) {
			window.myTouchButtons.align(); 
		}
...
MyCongrats = ig.Game.extend({
	...
	init: function() {
		...
		ig.input.bind( ig.GAMEPAD.FACE_3, 'shoot' );	
        ig.input.bind( ig.GAMEPAD.START, 'start');
        this.animSheet = new ig.AnimationSheet( 'media/congratulations.png', 960, 540);
		this.anim = new ig.Animation( this.animSheet, 0.2, [0] );
		// Align touch buttons to the screen size, if we have any
		if( window.myTouchButtons ) {
			window.myTouchButtons.align(); 
