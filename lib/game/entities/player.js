...
EntityPlayer = ig.Entity.extend({
        ...
        if( !ig.global.wm )    {
            ...
            ig.game.events.on("wave", function(){
                ...
                if(self.waveMode == -1){
                    ...
                    // we're in hover-mode
                    if(self.flightStamina > 0){
                        self.vel.y = -300;
                        self.flightStamina = self.flightStamina -1;
                    }
                }
            });
	...
	update: function() {
        if(this.pos.y > ig.game.screen.y +1000){
            this.kill();
        }
	...
	receiveDamage: function( amount, from ) {
		...
		// Knockback
		this.vel.x = (from.pos.x > this.pos.x) ? -600 : 600;
		this.vel.y = -300;
